...
function getDept(pageNum){
    $.ajax({
        url: 'http://localhost:8080/api/d1/deptpage?page='+pageNum,
        type : 'GET',
        dataType : 'json',
        success : function(response){
            ...
            for(var i=0; i<response.list.length; i++){
                html += '<tr onclick="getEmpByDeptno('+response.list[i].deptno+')"><td>'+response.list[i].deptno+'</td><td>'+response.list[i].dname+'</td><td>'+response.list[i].loc
...
//해당 부서 누르면 뿌려주게 
function getEmpByDeptno(deptno){
    console.log('클릭한 부서 번호는?' +deptno);
    $.ajax({
        url : 'http://localhost:8080/api/d1/dept/'+deptno,
        type : 'GET',
        dataType : 'json',
        success : function(response){
            console.log(response);
...
function setDept(){
    ...
    $.ajax({
        url : 'http://localhost:8080/api/d1/dept',
        type : 'POST',
...
function update(){
    ...
    $.ajax({
        url : 'http://localhost:8080/api/d1/dept',//주소 변경해야함. 백엔드에 있어야함.
        type : 'PATCH',
        contentType : 'application/json',
        dataType : 'json',
        data : JSON.stringify(jsonData),
