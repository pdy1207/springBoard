...
import com.portfolio.dyport.service.ExcelService;
...
public class ExcelController {
	...
	//HttpServletResponse 라는 자바 객체가 대신 리턴해주겠다.
	@GetMapping("/excel/download")
	public void downloadExcelFile(HttpServletResponse response) throws Exception{
		
		String today = new SimpleDateFormat("yyMMdd").format(new Date()); //자바 오늘 날짜 기입
		String title = "dy_사원_게시판"; //Excel 파일 이름
		
		response.setContentType("ms-vnd/excel");//엑셀 파일을 보내겠다.
        response.setHeader("Content-Disposition", "attachment;filename="
		+ URLEncoder.encode(today+"_"+title,"UTF-8")+".xls");//엑셀 파일이름 수정
